/** @file
    @brief	STM32F746G Discovery Initialize
...
#define PLL_M		25
//!!!#define PLL_N		400
#define PLL_N		324 //!!! SDRAMのデータバケが発生するので暫定対策
...
void init_system_drivers(void)
{
	...
	register_console_in_dev((struct st_device *)&uart1_device);
	register_error_out_dev(&uart1_low_device);

	// USART6
	register_device((struct st_device *)&uart6_device, 0);
...
//	register_device((struct st_device *)&gpio_keyboard_device, 0);
	...
	register_device((struct st_device *)&null_device, 0);
#endif
}
...
/* NOT API
   ...
   @note	プロセスAPIを必要とする初期化を行う
*/
void init_system_process(void)
{
#ifdef GSC_DEV_ENABLE_I2C
	// I2Cドライバ
	register_device((struct st_device *)&i2c1_device, 0);
	register_device((struct st_device *)&i2c4_device, 0);
#endif

#ifdef GSC_COMP_ENABLE_GRAPHICS
	// グラフィックスドライバ初期化
	register_device((struct st_device *)&lcd_device, 0);
	register_device((struct st_device *)&framebuf_device, 0);
	init_graphics("fb");	// グラフィックス初期化
#ifdef GSC_COMP_ENABLE_FONTS
#ifdef GSC_DEV_ENABLE_GRCONSOLE
	register_device((struct st_device *)&grconsole_device, 0);
#endif
#endif
	...
	register_device((struct st_device *)&sdmmc_device, 0);
#endif
	...
	// サウンド、オーディオドライバ登録
	register_device((struct st_device *)&audio_device, 0);
#endif
...
#include "device/buzzer_ioctl.h"
	...
	register_device((struct st_device *)&adc_device, 0);
#endif

#ifdef GSC_DEV_ENABLE_ETHER
	// Etherドライバ
	register_device((struct st_device *)&ether_device, 0);
#endif	
}
