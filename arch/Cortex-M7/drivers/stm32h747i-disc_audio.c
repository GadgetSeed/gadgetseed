/** @file
    @brief	STM32H747I-Discovery Audio Driver
...
static int audio_init(int vol, int smprate)
{
	...
	AudioOutInit.SampleRate = smprate;
	AudioOutInit.BitsPerSample = AUDIO_RESOLUTION_16B;
	AudioOutInit.Volume = vol;
...
static int audio_ioctl(struct st_device *dev, unsigned int com, unsigned int arg, void *param)
{
	...
	case IOCMD_AUDIO_SET_VOLUME:
		DKFPRINTF(0x01, "IOCMD_AUDIO_SET_VOLUME %u\n", arg);
		volume = arg;
		if(flg_play) {
			lock_ts();
	...
	case IOCMD_AUDIO_SET_BUFSIZE:
		DKFPRINTF(0x01, "IOCMD_AUDIO_SET_BUFSIZE %u\n", arg);
		if(arg <= MAX_AUDIOBUF) {
			if(bufsize != arg) {
				flg_bs_chg = 1;
				bufsize = arg;
				//eprintf("Next bufsize = %d\n", bufsize);
			}
		}
		break;

	case IOCMD_AUDIO_PLAY_START:
		DKFPRINTF(0x01, "IOCMD_AUDIO_PLAY_START(flg_play = %d)\n", flg_play);
		if((flg_play == 0) || (flg_bs_chg != 0)) {
			...
			if(flg_audio_start == 0) {
				...
				//eprintf("bufsize = %d\n", bufsize);
				lock_ts();
				rt = BSP_AUDIO_OUT_Play(0, (uint8_t*)audio_buf, bufsize);
				if(rt != BSP_ERROR_NONE) {
					SYSERR_PRINT("BSP_AUDIO_OUT_Play error(%d)\n", rt);
				}
				...
				if(flg_bs_chg == 0) {
					...
					int rt;
					// [TODO] 何故か最初期化しないとバッファサイズの変更を受け付けない？
					BSP_AUDIO_OUT_Stop(0);
					BSP_AUDIO_OUT_DeInit(0);
					BSP_I2C4_DeInit();
					BSP_I2C4_Init();
	...
	case IOCMD_AUDIO_PLAY_STOP:
		DKFPRINTF(0x01, "IOCMD_AUDIO_PLAY_STOP\n");
		if(flg_play != 0) {
			int i;
			lock_ts();
	...
	case IOCMD_AUDIO_SET_SMPRATE:
		DKFPRINTF(0x01, "IOCMD_AUDIO_SET_SMPRATE %d\n", arg);
		{
			if(sampling_rate != arg) {
				sampling_rate = arg;
				lock_ts();
				BSP_AUDIO_OUT_SetSampleRate(0, sampling_rate);
				unlock_ts();
			}
		}
		break;
	...
	case IOCMD_AUDIO_SET_MUTE:
		DKFPRINTF(0x01, "IOCMD_AUDIO_SET_MUTE %d\n", arg);
		{
			lock_ts();
			if(arg == 0) {
