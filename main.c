/** @file
    ...
    コンフィグレーションでは、ユーザは以下の２つの項目を選択します。

    - GadgetSeed を動作させる「ハードウェア(システム)」
    - GadgetSeed 上で実行する「アプリケーション」

    コンフィグレーションにより GadgetSeed のソースツリーのルートディレクトリに以下のファイルを作成します。
    ...
    config.mk は config.mk ファイルがない状態で make を実行すると表示されるメニューより、システム及びアプリケーションを選択することで作成することができます。
